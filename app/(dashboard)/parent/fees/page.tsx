...
import { CreditCard, Check, AlertCircle, Receipt, IndianRupee } from "lucide-react";
...
const pending = myFees.filter(f => f.status === "Pending");
...
const totalPending = pending.reduce((a, f) => a + f.amount, 0) + overdue.reduce((a, f) => a + f.amount, 0);

const feeTypeData = [
  { name: "Tuition", value: 12000 },
  { name: "Transport", value: 3000 },
  { name: "Sports", value: 1500 },
  { name: "Lab", value: 2000 },
  { name: "Library", value: 500 },
];
...
export default function ParentFeesPage() {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="xl:col-span-2">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Fee Details — {myChild.academicYear}</CardTitle>
            </CardHeader>
                        ...
                        <td className="py-3">
                          ...
                          {fee.receiptNumber && (
                            <p className="text-xs text-gray-400 flex items-center gap-0.5">
                              <Receipt className="w-3 h-3" /> {fee.receiptNumber}
                            </p>
                          )}
                        </td>
      ...
      {/* Pay Dialog */}
      <Dialog isOpen={payOpen} onClose={() => setPayOpen(false)} size="md">
        <DialogHeader>
