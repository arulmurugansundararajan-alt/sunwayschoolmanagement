...
export async function GET() {
  try {
    ...
    if (count === 0) {
      await RolePermissionModel.insertMany(DEFAULT_PERMISSIONS);
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (permission.isSystem) {
      return NextResponse.json({ success: false, message: "System roles (teacher, accountant) cannot be deleted" }, { status: 403 });
    }
